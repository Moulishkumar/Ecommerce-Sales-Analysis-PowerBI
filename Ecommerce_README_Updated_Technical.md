...
A **snowflake-style schema** is used, with the `2021 Sales` table as the central fact table and related dimension tables:

- **Fact Table** – `2021 Sales` containing sales transactions.
- **Dimension Table** – `Item-Vendor` containing item and vendor information.
- **Dimension Table** – `Vendor-Manager` containing vendor and sales manager information.
- **Calculated Table** – `Second best selling item` for second-best-selling item analysis.
- The dimension tables are connected to each other, with `Item-Vendor` linking to `Vendor-Manager` and `2021 Sales`.
...
- Data Modeling & Snowflake-style Schema Design.
- Advanced DAX.
- `RANKX`, `REMOVEFILTERS`, `ALL`, `AVERAGEX` and `DATESINPERIOD`.
